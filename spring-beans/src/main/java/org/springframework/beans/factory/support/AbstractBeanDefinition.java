...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * lenientConstructorResolution主要是判断解析构造函数的时候是否以宽松模式还是严格模式
	...
	/**
	 * 记录构造函数注入属性，对应bean属性construction-arg
	 */
	@Nullable
	...
	/**
	 * 是否是用户自己定义的而非应用程序本身定义的，创建aop的时候为true，程序设置
	 */
	private boolean synthetic = false;

	/**
	 * 定义这个bean的应用 , APPLICATION：用户；INFRASTRUCTURE，完全内部使用，与用户无关 ；SUPPORT：某些复杂配置的一部分程序设置
	 */
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		//如果当前类中的方法只有一个，那么就设置该方法没有被重载，这样在后续调用的时候便可以直接使用找到的方法，
		...
		//bean 实例化的时候如果检测到存在method Overrides 属性，会动态地为当前bean 生成代理
		// 并使用对应的拦截器为bean 做增强处理，
	}
