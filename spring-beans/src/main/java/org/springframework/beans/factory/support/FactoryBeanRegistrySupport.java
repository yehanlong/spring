...
public abstract class FactoryBeanRegistrySupport extends DefaultSingletonBeanRegistry {
	...
	protected Object getObjectFromFactoryBean(FactoryBean<?> factory, String beanName, boolean shouldPostProcess) {
		...
		if (factory.isSingleton() && containsSingleton(beanName)) {
			synchronized (getSingletonMutex()) {
				...
				if (object == null) {
					//TODO bean转换最终核心功能代码，通过factory.getObject获取
					object = doGetObjectFromFactoryBean(factory, beanName);
					// Only post-process and store if not put there already during getObject() call above
					// (e.g. because of circular reference processing triggered by custom getBean calls)
					//仅对上面的getObject（）调用期间尚未放置的内容进行后处理
					...
					if (alreadyThere != null) {
						...
						if (shouldPostProcess) {
							...
							// 将当前正要创建的bean 记录在缓存中，这样便可以对循环依赖进行检测。
							beforeSingletonCreation(beanName);
							try {
								//TODO factory.getObject（）得到我们想要的结果后并没有直接返回，而是接下来又做了些后处理的操作
	...
	private Object doGetObjectFromFactoryBean(final FactoryBean<?> factory, final String beanName)
		...
		// 这是因为在很多情况下，可能由于FactoryBean没有完全初始化完成，导致调用FactoryBean中的getObject方法返回为空。
		if (object == null) {
			if (isSingletonCurrentlyInCreation(beanName)) {
	...
	protected Object postProcessObjectFromFactoryBean(Object object, String beanName) throws BeansException {
		//TODO 调用的是重写的postProcessObjectFromFactoryBean里的 applyBeanPostProcessorsAfterInitialization()
		return object;
	}
