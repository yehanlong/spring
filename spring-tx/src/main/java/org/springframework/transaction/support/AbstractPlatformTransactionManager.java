...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		...
		//判断当前线程是否存在事务，
		//判断依据为当前线程记录的连接不为空
		//且连接中(connectionHolder)中的transactionActive 属性不为空
		if (isExistingTransaction(transaction)) {
			// Existing transaction found -> check propagation behavior to find out how to behave.
			//TODO 如果已经存在事务,事务的嵌套处理
		...
		//如果当前线程不存在事务，但是propagationBehavior 却被声明为PROPAGATION_MANDATORY
		//抛出异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			//PROPAGATION _REQUIRED 、PROPAGATION REQUIRES_NEW 、PROPAGATION_NESTED
			//都需要新建事务
			...
			try {
				...
				//TODO 构造transaction ，包括设置connectionHolder 、隔离级别、timeout 如果是新连接，绑定到当前线程
				doBegin(transaction, definition);
				//TODO 将事务信息记录在当前线程中,新同步事务的设置
	...
	private TransactionStatus handleExistingTransaction(
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			//如果当前TransactionManager不支持嵌套事务直接抛错
			...
			if (useSavepointForNestedTransaction()) {
				...
				//使用嵌套的begin、commit/rollback实现嵌套事务，MySql不支持，
				// 因为如果已经调用过begin，提交之前再调用begin操作，
				// MySql会隐式调用一次commit，不能达到嵌套事务的效果，
				// 这种方式某些数据库可能会支持，这里不做介绍
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			try {
				...
				//添加的TransactionSynchronization 中的对应方法的调用
				...
				//如果当前status有保存点，表示当前提交的是
				//嵌套在某个事务内的子事务，通过释放保存点提交
				...
				if (status.hasSavepoint()) {
					...
					if (status.hasTransaction()) {
						if (status.isLocalRollbackOnly() || isGlobalRollbackOnParticipationFailure()) {
							...
							//如果当前事务不是独立的事务，那么只能标记状态， 等事务链执行完毕后统一回滚
