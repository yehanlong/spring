...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		//TODO 解析属性getTransactionAttribute
		...
		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			//(译)使用getTransaction和commit / rollback调用进行标准事务划分。

			//TransactionInfo 与TransactionAttribute 并不相同，
			//TransactionInfo 中包含TransactionAttribute 信息，但是，除了TransactionAttribute 外
			//还有其他事务信息，例如PlatformTransactionManager 以及TransactionStatus 相关信息。

			//TODO 创建TransactionInfo,在目标方法执行前获取事务并收集事务信息
			...
			try {
				...
				//TODO 异常回滚，默认只对RuntimeException 回滚
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		// 如果没有名称指定则使用方法唯一标识,
		// 并使用DelegatingTransactionAttribute封装txAttr
		// 当前的实际类型是RuleBasedTransactionAttribute,是由获取事务属性时生成，主要用于数据承载，而这里之所以使用DelegatingTransactionAttribute进行封装，当然是提供了更多的功能
	...
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		...
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			//这里判断是否回滚默认的依据是抛出的异常是存是RuntimeException 或者是Error 的类型
			if (txInfo.transactionAttribute != null && txInfo.transactionAttribute.rollbackOn(ex)) {
				try {
					//TODO 根据TransactionStatus 信息进行回滚处理
