...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected final void initServletBean() throws ServletException {
		...
		try {
			//initWebApplicationContext 函数的主要工作就是创建或刷新WebApplicationContext实例并
			//对servlet功能所使用的变量进行初始化。
			//TODO 关键的初始化逻辑实现委托给了initWebApplicationContext()
	...
	protected WebApplicationContext initWebApplicationContext() {
		...
		//何以判定这个参数是通过构造函数初始化，而不是通过上一次的函数返回值初始化呢？
		// 在Web中包含SpringWeb的核心逻辑的DispatcherServlet只可以被声明为一次，
		// 在Spring中已经存在验证，所以这就确保了如果this.webApplicationContext !=null ，
		// 则可以直接判定this.webApplicationContext 已经通过构造函数初始化。


		//通过构造函数的注入进行初始化WebApplicationContext
		...
		//如果构造函数没有初始化WebApplicationContext，再尝试通过contextAttribute 进行初始化
		if (wac == null) {
		...
		//如果以上两种方式都没法创建WebApplicationContext实例，那么这里重新创建
		if (wac == null) {
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		//为了保证当前线程的LocaleContext 以及RequestAttributes
