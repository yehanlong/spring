...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	...
	public <T> T execute(PreparedStatementCreator psc, PreparedStatementCallback<T> action)
		...
		try {
			...
			//TODO 设置查询超时时间，最大行等参数（就是一开始那些成员变量）
			...
			// Release Connection early, to avoid potential connection pool deadlock
			// in the case when the exception translator hasn't been initialized yet.
			//TODO 释放数据库连接避免当异常转换器没有被初始化的时候出现潜在的连接池死锁
	...
	protected void applyStatementSettings(Statement stmt) throws SQLException {
		//setFetchSize 最主要是为了减少网络交互次数设计的。访问ResultSet 时，如果它每次只从
		//服务器上读取一行数据， 则会产生大量的开销。setFetchSize 的意思是当调用rs.next 时，
		//ResultSet会一次性从服务器上取得多少行数据回来，这样在下次rs.next 时， 它可以直接从内存中获取数
	...
	protected void handleWarnings(Statement stmt) throws SQLException {
		...
		if (isIgnoreWarnings()) {
			if (logger.isDebugEnabled()) {
				//SQLWarning 提供关于数据库访问警告信息的异常。这些警告直接链接到
				// 导致报告警告的方法所在的对象。警告可以从Connection 、Statement 和
				// ResultSet 对象中获得。试图在已经关闭的连接上获取警告将导致抛出异常。
				// 类似地，试图在已经关闭的语句上或已经关闭的结果集上获取警告也将导致抛出异常。
