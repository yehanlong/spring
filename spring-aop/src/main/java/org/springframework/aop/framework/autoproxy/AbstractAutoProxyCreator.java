...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	public Object postProcessAfterInitialization(@Nullable Object bean, String beanName) throws BeansException {
		if (bean != null) {
			//根据给定的bean的class和name构建出一个key，格式： beanClassName beanName
			Object cacheKey = getCacheKey(bean.getClass(), beanName);
			if (this.earlyProxyReferences.remove(cacheKey) != bean) {
				//TODO 如果它适合被代理，则需要封装指定bean
				return wrapIfNecessary(bean, beanName, cacheKey);
			}
		}
		return bean;
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName,
		...
		// 检查proxyTargetClass设置以及preserveTargetClass 属性
		...
		// TODO 封装advisors并加入到ProxyFactory 中
	...
	protected Advisor[] buildAdvisors(@Nullable String beanName, @Nullable Object[] specificInterceptors) {
		...
		for (int i = 0; i < allInterceptors.size(); i++) {
			...
			//所以非常有必要统一封装成Advisor来进行代理的创建

			//TODO 拦截器进行封装转化为Advisor
