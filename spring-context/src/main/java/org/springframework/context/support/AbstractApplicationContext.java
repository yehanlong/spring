...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			//TODO 准备刷新的上下文环境，例如对系统属性或者环境变量进行准备和验证
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			//TODO 初始化BeanFactory，并进行xml文件读取，复用BeanFactory所提供的一切特性，这步后可以进行对bean的提取等基础操作
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			//TODO 对BeanFactory进行各种功能的填充，@Autowired和@Qualifier这两个注解就是在这一步骤中添加的支持
			...
			try {
				...
				//TODO 激活各种BeanFactory处理器
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				//TODO 注册拦截Bean创建的Bean处理器，这里只是注册，真正的调用是getBean()的时候
				...
				// 重点，重点，重点
				// TODO 初始化所有剩余的 singleton beans（lazy-init 的除外）
				finishBeanFactoryInitialization(beanFactory);

				// Last step: publish corresponding event.
				//完成刷新过程，通知生命处理器lifecycleProcessor刷新过程，
	...
	protected void prepareRefresh() {
		...
		//留给子类覆盖
		//开放式的结构，用户可以根据自身的需要重写initPropertySources方法，并在方法中进行个性化的属性处理及设置。
		initPropertySources();

		// Validate that all properties marked as required are resolvable:
		// see ConfigurablePropertyResolver#setRequiredProperties
		//验证需要的属性文件是否都放入环境中，比如@Value("XXX"),而XXX不在yml中会抛出异常
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		//TODO 初始化BeanFactory，并进行xml的读取，并将得到的BeanFactory记录在当前实体的属性中
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//增加对属性编辑器的支持,这样支持自定义属性的注入，比如Date类型的
		...
		//添加BeanPostProcess,使用上下文回调配置bean 工厂
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		//也就是说，Spring IoC 容器允许BeanFactoryPostProcessor在容器实际实例化任何其他的bean之前读取配置元数据，并有可能修改它
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 首先，初始化名字为 conversionService 的 Bean。conversionService最有用的场景就是，它用来将前端传过来的参数和后端的 controller 方法上的参数进行绑定的时候用
		...
		// Instantiate all remaining (non-lazy-init) singletons.
		// TODO 开始初始化,又回到DefaultListableBeanFactory
